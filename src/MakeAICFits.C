...
RooAbsPdf* MakeAICFits::getBackgroundPdf(int type, RooRealVar* mass) {                  
  ...
  case kDoubleExp:                                                                                                                                           
    {                                                                                                                                                        
      ...
      RooRealVar* alpha2_2 = new RooRealVar("alpha2_2","alpha2_22",-0.1,-1.,0.);                              
  ...
  case kTripleExp:                                                                                                                                             
    {                                                                                                                                                          
      ...
      RooRealVar* alpha2_3 = new RooRealVar("alpha2_3","alpha2_3",-0.1,-1.,0.);                              
      RooRealVar* alpha3_3 = new RooRealVar("alpha3_3","alpha3_3",-0.1,-1.,0.);                              
  ...
  case kModifiedExp:                                                                                                                                           
    { // pdf = e^(alpha1*m^alpha2)                                                                                                                             
      RooRealVar *alpha1_4 = new RooRealVar("alpha1_4","",-1.,-10.,0.);                                    
      RooRealVar *alpha2_4 = new RooRealVar("alpha2_4","",0.5,0.,10.);                                     
      BkgShape = new RooGenericPdf("ModifiedExp","Background Model","exp(@0*@1^@2)",RooArgList(*alpha1_4,*mass,*alpha2_4));   
      break;                                                                                                                                                   
    }                   
  case kPoly:                                                                                                                                                  
    {                                                                                                                                                          
  ...
  case kDoublePow:                                                                                                                                             
    { //pdf = f*m^alpha_1 + (1-f)*m^alpha_2                                                                                                                    
      RooRealVar *alpha1_p2 = new RooRealVar("alpha1_p2","alpha1_p2",-4.,-10.,0.);                                    
      RooRealVar *alpha2_p2 = new RooRealVar("alpha2_p2","alpha2_p2",-3.9,-10.,0.);                                   
...
RooAbsPdf* MakeAICFits::makeBackgroundFits(int type) {
  ...
  RooFitResult *bkg_databkg = BkgModel->fitTo(*dc, RooFit::Save(kTRUE), RooFit::Strategy(2),RooFit::NumCPU(NUM_CPU),RooFit::Minos(kFALSE),RooFit::Extended(kTRUE));
...
void MakeAICFits::run() {
  dc = w->data("Data_Combined")->reduce("evtcat==evtcat::cat0");
  mass = w->var("mass");
  ...
  RooAbsPdf* doubExp = MakeAICFits::makeBackgroundFits(1);
  doubExp->SetNameTitle("doubExp","Double Exponential");

  RooAbsPdf* tripExp = MakeAICFits::makeBackgroundFits(2);
  tripExp->SetNameTitle("tripExp","Triple Exponential");

  RooAbsPdf* modExp  = MakeAICFits::makeBackgroundFits(3);
  modExp->SetNameTitle("modExp","Modified Exponential");

  RooAbsPdf* Poly    = MakeAICFits::makeBackgroundFits(4);
  ...
  RooAbsPdf* singPow = MakeAICFits::makeBackgroundFits(5);
  ...
  RooAbsPdf* doubPow = MakeAICFits::makeBackgroundFits(6);
  ...
  cv->Update();
  cv->SaveAs("HggAIC_composite.pdf");

  wout->import(*JointModelExt);
  print();
  // Save workspace to file
  TFile *outputFile = new TFile("Workspaces/HggAIC_workspace_cat0.root","RECREATE");
  outputFile->cd();
